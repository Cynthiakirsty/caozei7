...
from modules.data_loader import (
    FULL_COLUMNS, USER_COLUMNS, ensure_sample_files,
)
...
# 示例文件仅供下载和参考，不再自动载入页面。
ensure_sample_files(Path("data"))
if "full_data" not in st.session_state:
    st.session_state.full_data = pd.DataFrame(columns=FULL_COLUMNS)
    st.session_state.user_data = pd.DataFrame(columns=USER_COLUMNS)
    st.session_state.full_source_name = "未上传"
    st.session_state.user_source_name = "未上传"
